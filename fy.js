...
if (!t) {
	...
	function checkPlatform() {
		...
		jQuery(function () {
			...
			function findInScope(root, selector) {
			...
			function bindCollapsibleSection($section, headerSelector, bodySelector) {
				var $header = $section.find(headerSelector).first();
				var $body = $section.find(bodySelector).first();
				...
				function syncState() {
					...
					if (isExpanded) {
						$body.css({ maxHeight: 'none', overflow: 'visible' });
					} else {
						$body.css({ maxHeight: '0px', overflow: 'hidden' });
					}
				}

				if ($section.attr('data-collapsible-bound') === 'true') {
					syncState();
					return;
				}
				...
				function toggleSection() {
					...
					if (isExpanded) {
						$body.css('max-height', bodyEl.scrollHeight + 'px');
						bodyEl.offsetHeight;
				...
				$header.attr({ role: 'button', tabindex: '0' });
				$header.on('click', toggleSection);
				...
				$section.attr('data-collapsible-bound', 'true');
				syncState();
			}

			// 将每个词条分区包裹，统一控制标题区与内容区样式。
			function wrapDictSections(root) {
				// 收集所有包含 .hwrap 的容器，每个容器单独处理（兼容多 .mdict 结构）
				var processedContainers = [];
				findInScope(root, '.hwrap').each(function () {
				...
				function wrapGroup(group) {
					...
					var $phraseRankElements = $groupEl.filter('sup').filter(function () {
						var rankText = $(this).text().replace(/\s+/g, '');
						return /^\d+$/.test(rankText) && $(this).next().hasClass('epfd');
					});
					var $mainElements = $groupEl.filter('.epfd, .coca, .srank, .coca2, .gdc').add($phraseRankElements);
			...
			// Some dictionary hosts append a source after the heading has already
			// been wrapped. Attach those direct siblings to the preceding entry.
			function adoptLateEntryNodes(root) {
				if (!root || root === document) return;
				var selector = '.pf, .word-frequency, .epfd, .coca, .srank, .coca2, .gdc';
				findInScope(root, selector).filter(function () {
					return !$(this).closest('.dict-section-all').length;
				}).each(function () {
					var $node = $(this);
					var $entry = $node.prevAll('.dict-section-all').first();
					if (!$entry.length) return;
					var $target = $node.is('.pf, .word-frequency')
						? $entry.children('.dict-section-top').first()
						: $entry.children('.dict-section-main').first();
					if (!$target.length) {
						$target = $('<div></div>').addClass($node.is('.pf, .word-frequency') ? 'dict-section-top' : 'dict-section-main');
						$entry.append($target);
					}

					if ($node.hasClass('epfd')) {
						var $rank = $node.prev('sup').filter(function () {
							return /^\d+$/.test($(this).text().replace(/\s+/g, ''));
						});
						if ($rank.length && !$rank.closest('.dict-section-all').length) $target.append($rank);
					}
					$target.append($node);
				});
			}

			function renderEntrySeparators() {
				var parents = [];
				$('.dict-section-all').each(function () {
				...
				for (var parentIndex = 0; parentIndex < parents.length; parentIndex++) {
					...
					var $entries = $parent.children('.dict-section-all');

					$parent.children('.dict-entry-separator').each(function () {
						if (!$(this).next().is('.dict-section-all')) $(this).remove();
					});

					$entries.each(function (entryIndex) {
						var $entry = $(this);
						var $previousSeparator = $entry.prev('.dict-entry-separator');
						if (entryIndex === 0) {
							$previousSeparator.remove();
							return;
						}
						if ($previousSeparator.length) return;

						var separator = document.createElement('div');
						var label = document.createElement('span');
						separator.className = 'dict-entry-separator';
						separator.setAttribute('role', 'separator');
						separator.setAttribute('aria-label', '其他词条');
						label.textContent = '其他词条';
			...
			// 同一词条的全部 .epfd 共同组成一个短语词频组件
			function wrapPhraseFrequencySections(root) {
				var $entries = findInScope(root, '.dict-section-all');
				if (root && root !== document) {
					$entries = $entries.add($(root).closest('.dict-section-all'));
				}

				$entries.each(function () {
					var $entry = $(this);
					var $allEpfd = $entry.find('.epfd');
					var $contentEpfd = $allEpfd.filter(function () {
						var $epfd = $(this);
						var compactText = $epfd.text().replace(/[\s\u00A0\u200B-\u200D\uFEFF]+/g, '');
						return Boolean(compactText || $epfd.find('img, canvas, svg').length);
					});
					var $sections = $entry.find('.phrase-frequency-section');
					var $rankSource = $contentEpfd.first().prev('sup').filter(function () {
						return /^\d+$/.test($(this).text().replace(/\s+/g, ''));
					}).first();

					$allEpfd.not($contentEpfd).hide();
					if (!$contentEpfd.length) {
						if ($sections.length) $sections.first().before($allEpfd);
						$sections.remove();
						return;
					}
					...
					if (!$section.length) {
						...
						var phraseDefaultClass = window.phrase_freq_expand ? 'expanded' : 'collapsed';

						section.className = 'phrase-frequency-section ' + phraseDefaultClass;
						header.className = 'phrase-frequency-header';
						title.className = 'phrase-frequency-title';
						title.textContent = 'Phrase Frequency';
						body.className = 'phrase-frequency-body';

						header.appendChild(title);
						section.appendChild(header);
						section.appendChild(body);
						$contentEpfd.first().before(section);
						$section = $(section);
					}

					var $body = $section.find('.phrase-frequency-body').first();
					$contentEpfd.each(function () {
						if (this.parentNode !== $body[0]) $body[0].appendChild(this);
					});
					$sections.not($section).remove();

					var $header = $section.find('.phrase-frequency-header').first();
					if (!$header.find('.phrase-frequency-rank').length && $rankSource.length) {
						var rank = document.createElement('span');
						var rankLabel = document.createElement('span');
						var rankValue = document.createElement('span');
						rank.className = 'phrase-frequency-rank';
						rankLabel.className = 'phrase-frequency-rank-label';
						rankValue.className = 'phrase-frequency-rank-value';
						rankLabel.textContent = 'RANK';
						rankValue.textContent = $rankSource.text().replace(/\s+/g, '');
						rank.appendChild(rankLabel);
						rank.appendChild(rankValue);
						$header.find('.phrase-frequency-title').first().after(rank);
						$rankSource.remove();
					}
			...
			wrapPhraseFrequencySections();

			// Stable Lookup 的唯一单词词频渲染路径。
			var POS_LABELS = {
				n: 'NOUN.', noun: 'NOUN.',
				v: 'VERB.', verb: 'VERB.',
				j: 'ADJ.', adj: 'ADJ.', adjective: 'ADJ.',
				r: 'ADV.', adv: 'ADV.', adverb: 'ADV.',
				c: 'CONJ.', conj: 'CONJ.', conjunction: 'CONJ.',
				i: 'PREP.', prep: 'PREP.', preposition: 'PREP.',
				d: 'DET.', det: 'DET.', determiner: 'DET.',
				a: 'ART.', art: 'ART.', article: 'ART.',
				m: 'NUM.', num: 'NUM.', number: 'NUM.',
				p: 'PRON.', pron: 'PRON.', pronoun: 'PRON.',
				t: 'INF.', inf: 'INF.', infinitive: 'INF.',
				u: 'INT.', int: 'INT.', interjection: 'INT.',
				x: 'NEG.', neg: 'NEG.', negation: 'NEG.',
				e: 'EXIST.', exist: 'EXIST.', existential: 'EXIST.',
				g: 'GEN.', gen: 'GEN.', genitive: 'GEN.'
			};

			function cleanMetricText(value) {
				return $.trim(String(value == null ? '' : value).replace(/[\u00A0\u200B-\u200D\uFEFF]/g, ''));
			}

			function normalizePosLabel(value) {
				var raw = cleanMetricText(value).replace(/\.+$/, '').toLowerCase();
				if (!raw) return '';
				return POS_LABELS[raw] || raw.toUpperCase() + '.';
			}

			function numericMetric(value) {
				var normalized = cleanMetricText(value).replace(/,/g, '');
				...
				var rows = [];
				var rowCount = Math.max($positions.length, $ranks.length, $totals.length);

				for (var rowIndex = 0; rowIndex < rowCount; rowIndex++) {
					var rankText = cleanMetricText($ranks.eq(rowIndex).text());
					var freqText = cleanMetricText($totals.eq(rowIndex).text());
					if (!rankText && !freqText) continue;
					rows.push({
						pos: isSpoken ? '' : normalizePosLabel($positions.eq(rowIndex).text()),
						rank: rankText,
						freq: freqText,
						order: rowIndex
					});
				...
				if (!isSpoken) {
					rows.sort(function (left, right) {
						var leftRank = numericMetric(left.rank);
						var rightRank = numericMetric(right.rank);
						if (isNaN(leftRank)) leftRank = Number.MAX_VALUE;
						if (isNaN(rightRank)) rightRank = Number.MAX_VALUE;
						return leftRank === rightRank ? left.order - right.order : leftRank - rightRank;
					});
				}

					$source.children('.frequency-row').remove();
					for (var outputIndex = 0; outputIndex < rows.length; outputIndex++) {
					var rowData = rows[outputIndex];
					var $row = $('<div class="frequency-row"></div>');
					if (!isSpoken) {
						var posKey = rowData.pos.replace(/\.$/, '').toLowerCase();
						$row.append($('<span class="frequency-pos"></span>').attr('data-pos', posKey).text(rowData.pos));
					}
					$row.append($('<span class="frequency-rank"><span class="frequency-field">RANK</span><span class="frequency-value"></span></span>').find('.frequency-value').text(rowData.rank).end());
					$row.append($('<span class="frequency-freq"><span class="frequency-field">FREQ</span><span class="frequency-value"></span></span>').find('.frequency-value').text(rowData.freq).end());
						$source.append($row);
				}
				...
				$entries.each(function () {
					var $entry = $(this);
					var $main = $entry.children('.dict-section-main').first();
					if (!$main.length) return;

					var $oldSections = $main.find('.coca-srank-section');
					...
					var $detachedSources = $rawSources.detach();
					$oldSections.remove();
					var section = document.createElement('section');
					section.className = 'coca-srank-section ' + (window.freq_expand ? 'expanded' : 'collapsed');
					section.setAttribute('data-stable-frequency', 'true');
					var header = document.createElement('div');
					header.className = 'coca-srank-header';
					var title = document.createElement('span');
					title.className = 'coca-srank-title';
					title.textContent = 'Word Frequency';
					var body = document.createElement('div');
					body.className = 'coca-srank-body';
					header.appendChild(title);
					section.appendChild(header);
					section.appendChild(body);
					var $section = $(section);
					...
					for (var sourceIndex = 0; sourceIndex < sourceOrder.length; sourceIndex++) {
						...
						var $matching = $detachedSources.filter(function () {
							if (sourceKey === 'spoken') return $(this).hasClass('srank');
			...
			function renderCoca2Pills(root) {
				findInScope(root, '.coca2').each(function () {
					var $el = $(this);
					if ($el.attr('data-pills-rendered') === 'true') return;
					var text = $el.text();
					var regex = /([^()]+)\((\d+(?:\.\d+)?%)\)/g;
					var match;
					...
					while ((match = regex.exec(text)) !== null) {
						...
						pill.className = 'coca2-pill';
						labelEl.className = 'coca2-pill-label';
						percentEl.className = 'coca2-pill-percent';
						labelEl.textContent = label;
						percentEl.textContent = match[2];
						pill.appendChild(labelEl);
						pill.appendChild(percentEl);
					...
					if (hasMatch) {
						...
						$el.attr('data-pills-rendered', 'true');
					}
				});
			}
			renderCoca2Pills();
			...
			// 汉化考试等级标签
			function localizeExamLabels(root) {
				if (typeof document === 'undefined') {
					return;
				}
				var labels = findInScope(root, '.word-frequency span').toArray();
			...
			function detectDarkMode() {
				...
				if (isEudic) {
					...
					if (typeof MutationObserver !== 'undefined' && !$eudicParent.attr('observer-attached') && $eudicParent[0]) {
					...
					if (typeof MutationObserver !== 'undefined' && document.body) {
						new MutationObserver(function (mutations) {
							for (var mutationIndex = 0; mutationIndex < mutations.length; mutationIndex++) {
								if (mutations[mutationIndex].attributeName === 'class') updateEudicTheme();
							}
			...
			window.adoptLateEntryNodes = adoptLateEntryNodes;
	...
	jQuery(function () {
		(function ($) {
			// COCA frequency details intentionally remain hidden in Stable Lookup.
			$(".coca .table").hide().attr("aria-hidden", "true");
		})(jQuery);
	});
}
t = 1;
