...
5. 人工查看代表性的 320px、390px 与 720px 截图；
6. 对真实 MDX 制作 Android 测试包并完成目标词条实机验收。
...
- 齿轮放在 `The Little Dict · Refreshed` 右侧、宿主折叠箭头左侧；使用靠右展开的轻量浮层，不使用底部抽屉。
- 浮层不显示“显示设置”标题和关闭按钮；点击齿轮或浮层外部关闭。
- 只提供 `iWeb`、`COCA`、`Spoken`、`Phrase Frequency`、`考试标签与星级` 五项独立开关，不提供总开关。
- 当前查询页不存在某类真实内容时不显示对应设置行；数据存在但用户已关闭时，设置行必须保留以便重新开启。
- 五项全部不可用时保留齿轮，浮层显示 `当前词条暂无可配置内容`。
