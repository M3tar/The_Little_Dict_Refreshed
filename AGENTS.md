...
- 下一阶段：页面显示设置、简洁查询模式、恢复默认值和偏好持久化。
...
- 不使用负 margin、绝对定位覆盖层或固定词条名称修复布局。
...
3. 核对全部主题、视口和配置组合通过；
4. 人工查看代表性的 320px、390px 与 720px 截图；
5. 对真实 MDX 制作 Android 测试包并完成目标词条实机验收。
...
- 简洁查询模式具体隐藏哪些模块；
- 关闭简洁模式后如何恢复此前的独立选择；
- 页面偏好与 `config.ini` 默认值的优先级；
- 欧路 Android/iOS 对 `localStorage` 或其他持久化方式的支持与作用域；
- 不支持持久化时的降级行为。

上述问题未确认前，可以调研和制作原型，但不要把未经确认的交互直接写入正式代码。
