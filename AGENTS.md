...
- 当前稳定版本：`v0.2.1`，Git 标签 `v0.2.1`；在 `v0.2.0` 基础上补充 macOS 欧路桌面端设置入口兼容，完整词典包通过同版本 GitHub Release 分发。
- 上一稳定版本：`v0.2.0`，Git 标签 `v0.2.0`；`v0.1.0` 为更早的稳定查询版本。
- 当前稳定能力：可靠查询单词、短语、词频及相邻同形词，以及五项页面显示设置与偏好持久化。
- v0.2.0 完整包已在 Android 与 iOS 实机稳定运行；Android 已完成浅色、深色和设置交互截图核对。
- `b08` 已在 macOS 欧路桌面端完成实机验收：标题、原生折叠、设置入口、五项开关、外部点击关闭和后续查询持久化均正常；首次更新后若仍显示旧界面，应先清理欧路缓存再判断版本。
...
- 桌面欧路的词典分组标题结构是 `.expHead > a`，折叠图标位于链接内部；设置入口挂载时必须保留原链接、折叠图标和点击行为，并把设置按钮作为标题链接外的同级元素。
...
7. 对真实 MDX 制作测试包，并在本轮影响到的 Android、iOS 或桌面欧路宿主中完成目标词条实机验收。
...
浏览器 fixture 只是快速回归，不能替代欧路 Android、iOS、macOS/Windows 目标宿主与真实 `TLD.mdx` 的最终验收。
